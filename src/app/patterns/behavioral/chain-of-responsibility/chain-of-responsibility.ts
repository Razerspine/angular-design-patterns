import {Component, inject, WritableSignal} from '@angular/core';
import {CommonModule} from '@angular/common';
import {CodePreview} from "@shared/code-preview/code-preview";
import {TranslocoPipe} from "@ngneat/transloco";
import {FileLoaderService} from '@core/services/file-loader.service';
import {PreviewDataType} from '@core/models/preview-data';
import {CHAIN_OF_RESPONSIBILITY_CONFIG} from '@core/configs/patterns/behavioral';

@Component({
  selector: 'app-chain-of-responsibility',
  imports: [CommonModule, CodePreview, TranslocoPipe],
  templateUrl: './chain-of-responsibility.html',
  styleUrl: './chain-of-responsibility.scss'
})
export class ChainOfResponsibility {
  private fileLoader = inject(FileLoaderService);
  previewData: WritableSignal<PreviewDataType[]> = this.fileLoader.loadFiles(CHAIN_OF_RESPONSIBILITY_CONFIG);
}
